...
async def test_local_shell_accepts_wrapped_tool_arguments(
    ...
    event.role = role
    # The role-based Local permission model resolves execution rights from this
    # config; without it every role is denied and the wrapper normalization
    # under test is never reached.
    context = SimpleNamespace(
        get_config=lambda **kwargs: {
            # Admin execution is additionally gated on the sender being a
            # configured administrator, so the admin case needs this list.
            "admins_id": ["user"],
            "provider_settings": {
