...
7. 网络客户端应在插件初始化时创建并复用，设置明确的连接数、并发数和超时上限，并在 `terminate()` 中关闭。不要在每次消息处理时新建客户端，也不要让插件重试与 SDK 重试叠加。
8. 如果是对某个插件进行功能扩增，请优先给那个插件提交 PR 而不是单独再写一个插件（除非原插件作者已经停止维护）。
9. 如果直接借鉴了其他项目的设计、功能创意或实现思路，请在 README 中清楚说明灵感来源并附上相关项目链接。
10. 如果使用、修改或移植了其他项目的代码或资源，请遵守原项目的开源许可协议，并按协议要求保留版权及许可声明。
...
async def test(self, event: AstrMessageEvent):
    ...
    prov = await self.context.get_using_provider_async(umo=event.unified_msg_origin)
    if prov:
