...
- Create and reuse network clients during plugin initialization, set explicit connection, concurrency, and timeout bounds, and close them in `terminate()`. Do not create a client for every message or stack plugin retries on top of SDK retries.
